...
/**
 * kd_tree class: implemented using points in Dim dimensional space (given by the template parameter).
 */
template <int Dim>
class kd_tree
{
  public:
     ...
     */
    bool smaller_in_dimension(const point<Dim>& first, const point<Dim>& second, int dim) const;
    /**
     ...
     * should_replace(target, curr_best1, poss_best3); // based on operator<, this should be false
     * \endcode
     *
     * @param target The point we want to be close to.
     * @param current_best The point that is currently our closest point to target.
     ...
     * target than current_best. Ties should be broken with point::operator<().
     */
    bool should_replace(const point<Dim>& target, const point<Dim>& current_best, const point<Dim>& potential) const;
    /**
     ...
     * represent the tree as a linear structure. For testing, we **require** the following:
     *
     ...
     * already wrote...
     *
     * @see [Partition-based General Selection
     ...
     * valid kd-tree.
     *
     * @see Here is a reference for kd-tree:
     ...
     * (https://wiki.engr.illinois.edu/display/cs225/MP+6.1) on the kd_tree spec on the wiki.
     *
    ...
    /**
	* helper function which finds the squared Euclidean distance between two points 	
	*/
    int sDistance(const point<Dim>& p1, const point<Dim>& p2) const;
    
    /*
    * helper function to construct a kd tree.
    */
    void construct(size_t start, size_t end, int depth);

    /*
    * helper function which rearranges the points such that points to left of
    * of median are smaller than median and points to the right of median are
    * larger than median, where the median is chosen along the axis dimension.
    */
    void select(int axis, size_t start, size_t end, size_t median);

    /*
    * helper function to implement the select function.
    */
    size_t partition(int axis, size_t start, size_t end, size_t pivotIdx);

	/*
    * helper function to find nearest neighbor.
    */
    void find_nearest_neighbor(const point<Dim>& query, int& cur_best, size_t start, size_t end, int depth) const;     
};
