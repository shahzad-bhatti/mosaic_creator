 ...
 * sourceimage.h
 *
 * source_image extends the epng class and provides some additional data and
...
int source_image::rows() const
{
    if (backing_image_.height() <= backing_image_.width())
        ...
        return divide(resolution_ * backing_image_.height(), backing_image_.width());
}
...
int source_image::columns() const
{
    if (backing_image_.width() <= backing_image_.height())
        ...
        return divide(resolution_ * backing_image_.width(), backing_image_.height());
}
